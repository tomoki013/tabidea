...
### 2026-03-19
- `local` fix(compose-pipeline,runtime-budget): timeout 設計を再整理。① 「8.5秒は短すぎないか？」という観点に対し、seed/spots のアプリ内 deadline を固定値ではなく route `maxDuration=9` に連動する共有 runtime budget へ変更。② 9秒自体は Netlify free plan の 10 秒制限を安全に踏まえるため維持しつつ、seed は `9_000ms - 200ms reserve`、spots は `9_000ms - 500ms reserve` まで使えるようにして、不要に早い打ち切りをなくした。③ `/api/itinerary/plan/*` と legacy `/api/itinerary/compose` の `maxDuration` も共通定数参照へ統一し、実行時間の前提が route と orchestrator でズレないよう修正。
