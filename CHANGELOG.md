...
- `local` refactor(compose): rebase 競合を減らすため deterministic fallback 実装を `pipeline-orchestrator.ts` から `steps/deterministic-semantic-fallback.ts` へ分離。オーケストレーター本体の差分面積を縮小し、同ファイルへの同時変更時でも rebase 衝突を起こしにくい構成に整理。
