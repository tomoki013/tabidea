...
-- Purpose: allow fresh databases to be bootstrapped via supabase/migrations in order.
-- This file must remain idempotent because existing environments may apply it after later migrations exist.
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_plans_public ON plans(is_public, created_at DESC) WHERE is_public = TRUE;
CREATE INDEX IF NOT EXISTS idx_plans_key_version ON plans(key_version) WHERE key_version < (SELECT MAX(version) FROM encryption_key_versions WHERE status = 'active');
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'plans'
      AND policyname = 'Anyone can view public plans'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'plans'
      AND policyname = 'Users can view own plans'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'plans'
      AND policyname = 'Users can insert own plans'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'plans'
      AND policyname = 'Users can delete own plans'
  ) THEN
...
CREATE INDEX IF NOT EXISTS idx_saved_plans_user ON user_saved_plans(user_id);
...
BEGIN
  IF NOT EXISTS (
    ...
    FROM pg_policies
    WHERE schemaname = 'public'
      AND tablename = 'user_saved_plans'
      AND policyname = 'Users can manage own saved plans'
  ) THEN
...
-- 4. ENTITLEMENT TYPES
-- ============================================
CREATE TABLE IF NOT EXISTS entitlement_types (
...
CREATE INDEX IF NOT EXISTS idx_grants_user ON entitlement_grants(user_id);
CREATE INDEX IF NOT EXISTS idx_grants_user_type ON entitlement_grants(user_id, entitlement_type);
CREATE INDEX IF NOT EXISTS idx_grants_active ON entitlement_grants(user_id, status, valid_until);
CREATE INDEX IF NOT EXISTS idx_grants_source ON entitlement_grants(source_type, source_id);
...
BEGIN
  IF NOT EXISTS (
    ...
    FROM pg_policies
    WHERE schemaname = 'public'
      AND tablename = 'entitlement_grants'
      AND policyname = 'Users can view own grants'
  ) THEN
...
CREATE INDEX IF NOT EXISTS idx_ticket_packs_user ON ticket_packs(user_id);
...
BEGIN
  IF NOT EXISTS (
    ...
    FROM pg_policies
    WHERE schemaname = 'public'
      AND tablename = 'ticket_packs'
      AND policyname = 'Users can view own ticket packs'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'entitlement_grants_updated_at'
      AND tgrelid = 'entitlement_grants'::regclass
  ) THEN
