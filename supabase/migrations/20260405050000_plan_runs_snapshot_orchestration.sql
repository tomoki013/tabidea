alter table public.plan_runs
  ...
  add column if not exists current_pass_id text,
  add column if not exists last_completed_pass_id text,
  add column if not exists execution_lease_token text,
  add column if not exists execution_lease_expires_at timestamptz,
  add column if not exists active_slice_id text,
  add column if not exists resume_hint jsonb not null default '{"mode":"none"}'::jsonb;

update public.plan_runs
set
  current_pass_id = coalesce(current_pass_id, active_pass_id),
  execution_lease_token = coalesce(execution_lease_token, lease_token),
  execution_lease_expires_at = coalesce(execution_lease_expires_at, lease_expires_at)
where current_pass_id is null
   or execution_lease_token is null
   or execution_lease_expires_at is null;

update public.plan_runs
set
  pause_context = coalesce(
    pause_context,
    jsonb_build_object(
      'pauseReason', 'infrastructure_interrupted',
      'resumePassId',
        case state
          when 'created' then 'normalize_request'
          when 'request_normalized' then 'plan_frame_build'
          when 'frame_built' then 'draft_generate'
          when 'draft_generated' then 'draft_validate'
          when 'draft_validated' then 'timeline_finalize'
          when 'draft_repaired' then 'draft_validate'
          when 'timeline_finalized' then 'completion_gate'
          when 'gate_passed' then 'persist_completed_trip'
          else current_pass_id
        end,
      'pausedAt', coalesce(updated_at, now())
    )
  ),
  state = 'paused'
where state in (
...
update public.plan_runs
set
  pause_context = jsonb_build_object(
    'pauseReason', 'infrastructure_interrupted',
    'resumePassId', coalesce(current_pass_id, 'normalize_request'),
    'pausedAt', coalesce(updated_at, now())
  ),
  resume_hint = jsonb_build_object(
    'mode', 'manual',
...
where state = 'created'
  and coalesce(execution_lease_expires_at, lease_expires_at) is not null
  and coalesce(execution_lease_expires_at, lease_expires_at) < now();

update public.plan_runs
set resume_hint =
  case
    when state = 'paused' and coalesce(pause_context->>'pauseReason', '') in ('runtime_budget_exhausted', 'day_unit_boundary')
      then jsonb_build_object('mode', 'auto', 'reason', pause_context->>'pauseReason', 'retryAfterMs', 750)
    when state = 'paused'
      then jsonb_build_object('mode', 'manual', 'reason', coalesce(pause_context->>'pauseReason', 'infrastructure_interrupted'))
    else '{"mode":"none"}'::jsonb
  end;

alter table public.plan_runs
  drop constraint if exists plan_runs_state_check;

alter table public.plan_runs
  add constraint plan_runs_state_check check (
    state in ('created', 'running', 'paused', 'completed', 'failed')
  );

create index if not exists plan_runs_execution_lease_expires_at_idx
  on public.plan_runs(execution_lease_expires_at);

create table if not exists public.plan_run_slices (
...
create index if not exists plan_run_slices_run_id_started_at_idx
  on public.plan_run_slices(run_id, started_at desc);
...
declare
  ...
  v_lease_token text := encode(gen_random_bytes(16), 'hex');
...
begin
  ...
  insert into public.plan_run_slices (
    ...
    p_run_id,
    coalesce(v_before.active_slice_id, encode(gen_random_bytes(8), 'hex')),
    v_before.state,
    v_after.state,
    p_current_pass_id,
    p_stop_reason,
