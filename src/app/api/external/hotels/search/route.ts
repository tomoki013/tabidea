...
import { hotelSearchSchema } from '@/lib/external-providers/schema';
import {
  buildRequestHash,
  getProvider,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const candidates = await provider.searchHotels(criteria);
    ...
    console.error('[external/hotels/search] failed', error instanceof Error ? error.message : error);
    return NextResponse.json(
      {
        success: false,
        error: '外部ホテル検索に失敗しました。条件を調整して再試行してください。',
      },
      { status: 500 },
    );
  }
