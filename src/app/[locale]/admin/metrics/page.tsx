...
const KPI_KEY_MAP: Record<string, string> = {
  plan_to_action_rate: 'kpiLabels.plan_to_action_rate',
  replan_rate: 'kpiLabels.replan_rate',
  rescue_success_rate: 'kpiLabels.rescue_success_rate',
  reusage_intent_rate: 'kpiLabels.reusage_intent_rate',
  companion_share_rate: 'kpiLabels.companion_share_rate',
};

export default async function MetricsPage() {
  const language = await getRequestLanguage();
  const t = await getTranslations('pages.admin.metrics');
    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      {!summary ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <MetricCard
            label={t('cards.totalGenerations.label')}
            value={summary.totalGenerations.toString()}
            unit={t('cards.totalGenerations.unit')}
          />
          <MetricCard
            label={t('cards.avgTotalTime.label')}
            value={(summary.avgTotalTime / 1000).toFixed(1)}
            unit={t('cards.avgTotalTime.unit')}
          />
          <MetricCard
            label={t('cards.validationPassRate.label')}
            value={(summary.avgValidationPassRate * 100).toFixed(1)}
            unit="%"
          />
          <MetricCard
            label={t('cards.citationRate.label')}
            value={(summary.avgCitationRate * 100).toFixed(1)}
            unit="%"
          />
          <MetricCard
            label={t('cards.avgUserRating.label')}
            value={summary.avgUserRating > 0 ? summary.avgUserRating.toFixed(1) : t('naValue')}
            unit={summary.avgUserRating > 0 ? '/ 5' : ''}
          />
          <MetricCard
            label={t('cards.reportedIssues.label')}
            value={summary.totalIssues.toString()}
            unit={t('cards.reportedIssues.unit')}
          />
        </div>
      )}
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {kpis.map((kpi) => (
              <MetricCard
                key={kpi.name}
                label={KPI_KEY_MAP[kpi.name] ? t(KPI_KEY_MAP[kpi.name]) : kpi.name}
