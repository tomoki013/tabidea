...
function renderContentWithEmbeds(content: string, language: 'ja' | 'en') {
  ...
  while ((match = regex.exec(content)) !== null) {
    ...
    nodes.push(
      <iframe
        ...
        src={`${localizePath(`/shiori/${slug}`, language)}${token ? `?t=${token}` : ''}`}
  ...
  return (
    <main className="max-w-3xl mx-auto px-4 py-24 space-y-6">
      ...
      <article className="prose prose-stone max-w-none">
        {renderContentWithEmbeds(post.content_html || '', language)}
      </article>
    </main>
  );
}
