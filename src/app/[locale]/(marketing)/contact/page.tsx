...
export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations("pages.marketing.contact.meta");
  return {
    title: t("title"),
        ...
        <div className="space-y-12">
          ...
          <section className="text-center text-sm text-stone-500 space-y-2">
            ...
            <div className="flex flex-wrap items-center justify-center gap-3">
              <Link href={localizePath("/terms", language)} className="underline hover:text-[#e67e22]">{t("terms")}</Link>
              <span>|</span>
              <Link href={localizePath("/privacy", language)} className="underline hover:text-[#e67e22]">{t("privacy")}</Link>
              <span>|</span>
              <Link href={localizePath("/ai-policy", language)} className="underline hover:text-[#e67e22]">{t("aiPolicy")}</Link>
              <span>|</span>
              <Link href={localizePath("/cookie-policy", language)} className="underline hover:text-[#e67e22]">{t("cookiePolicy")}</Link>
            </div>
          </section>
