...
export const TransitInfoInputSchema = z.object({
  ...
  departure: z.object({
    place: z.string().describe('出発地'),
    time: z.string().nullable().describe('出発時刻（例: "09:00"）'),
  }),
  arrival: z.object({
    place: z.string().describe('到着地'),
    time: z.string().nullable().describe('到着時刻（例: "12:30"）'),
  }),
  duration: z.string().nullable().describe('所要時間（例: "3h 30m"）'),
  memo: z.string().nullable().describe('メモ（便名など）'),
});
...
export const ActivityInputSchema = z.object({
  ...
  activity: z.string().describe('アクティビティ名'),
  description: z.string().describe('詳細な説明（1-2文）'),
  // Places API fields — kept for search accuracy
  searchQuery: z.string().nullable().describe('Google Places API検索に最適化された英語の正式名称（例: "Kinkaku-ji Temple", "Ichiran Ramen Shibuya"）'),
  locationEn: z.string().nullable().describe('英語での場所名（例: "Kyoto, Japan"）— Places API地域フィルタ用'),
  // Fields below are set via post-processing (not generated by AI)
  activityType: z.enum(['spot', 'transit', 'accommodation', 'meal', 'other']).nullable().describe('アクティビティの種類'),
  source: z.string().nullable().describe('情報源'),
});
...
export const DayPlanInputSchema = z.object({
  day: z.number().int().min(1).describe('日数（1から開始）'),
  title: z.string().describe('日のタイトル・テーマ'),
  transit: TransitInfoInputSchema.nullable().describe('その日の主要な移動'),
  activities: z.array(ActivityInputSchema).min(1).describe('アクティビティ一覧'),
  // Fields below are removed from AI generation (set via post-processing or unused):
  // - timelineItems: buildTimeline() constructs from transit + activities
  // - ui_type: never consumed by UI
  // - reference_indices: kept at response-level only
});
...
export const PlanOutlineDaySchema = z.object({
  ...
  travel_method_to_next: z.string().nullable().describe('翌日への移動手段（最終日はnull）'),
});
...
export const DayPlanArrayResponseInputSchema = z.object({
  ...
  reference_indices: z.array(z.number()).nullable().describe('参考記事のインデックス'),
});
