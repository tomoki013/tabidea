...
/**
 * GoogleMapView — Google Maps 表示 (Premium/Admin ティア)
 *
 * 既存の DayMap.tsx からリファクタリング。
      ...
      <MapErrorBoundary className="w-full h-48 sm:h-56 md:h-64">
        <APIProvider apiKey={apiKey}>
          <div className="relative w-full h-48 sm:h-56 md:h-64 bg-stone-100">
              ...
              {selectedMarker && (
                ...
                >
                  <div className="min-w-[180px] p-1">
                    <h3 className="font-bold text-xs text-stone-800 mb-1 line-clamp-2">
                      {selectedMarker.name}
                    </h3>
                    <a
                      href={
            ...
            {/* Map Legend */}
            <div
              className={`absolute bottom-2 left-2 bg-white/90 backdrop-blur-sm rounded-lg px-3 py-1.5 text-xs text-stone-600 shadow-sm border border-stone-200/50 transition-opacity duration-300 ${isMapLoaded ? "opacity-100" : "opacity-0"}`}
            >
