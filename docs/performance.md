...
目標時間はモデル別に設定されています。Flash (デフォルト) と Pro で異なります。

### プラン概要生成 (`generatePlanOutline`)

| ステップ | Flash 目標 (ms) | Pro 目標 (ms) | 説明 |
|----------|----------------|---------------|------|
| `usage_check` | 500 | 500 | 利用制限チェック (Supabase) |
| `cache_check` | 300 | 300 | Redis キャッシュ確認 |
| `rag_search` | 2,000 | 2,000 | Pinecone ベクトル検索 + ユーザー制約取得 (並列) |
| `prompt_build` | 100 | 100 | プロンプト文字列構築 |
| `ai_generation` | 15,000 | 30,000 | Gemini API 概要生成 |
| `hero_image` | 2,000 | 2,000 | Unsplash 画像取得 |
| `cache_save` | 500 | 500 | Redis キャッシュ保存 (非同期) |
| **total** | **20,000** | **35,000** | **全体** |

### プラン詳細チャンク生成 (`generatePlanChunk`)

| ステップ | Flash 目標 (ms) | Pro 目標 (ms) | 説明 |
|----------|----------------|---------------|------|
| `prompt_build` | 100 | 100 | プロンプト構築 + ユーザー制約取得 |
| `ai_generation` | 20,000 | 35,000 | Gemini API 詳細生成 |
| **total** | **22,000** | **37,000** | **全体** |

### End-to-End 目標 (3日間プラン・Flash)
...
4. **maxOutputTokens の設定**
   - 全 `generateObject` 呼び出しに `maxTokens` を設定
   - Outline: 2,048、DayDetails: 4,096、Modify: 8,192、Legacy: 4,096
   - 無制限出力による長時間生成・タイムアウトを防止

5. **プロンプトサイズの削減**
   - Golden Plan 例を4個→2個に削減 (~6KB節約)
   - RAG記事の本文上限を500→300文字に短縮
   - Day Details 指示文を簡潔化 (78行→約50行)
   - 重複していた fixedSchedulePrompt を修正

6. **リトライ/タイムアウトの最適化**
   - リトライ遅延: 1,000ms → 500ms
   - 生成タイムアウト: 90秒 → 60秒

7. **モデル別パフォーマンス目標**
   - Flash/Pro でそれぞれ異なる目標時間を設定
   - AI生成後に `timer.setTargets()` でモデルに応じた目標に切り替え
