...
補足（サンプル再生成）:
- `src/scripts/generate-sample-itineraries.ts` は本番同等の outline + chunk フローを再利用してサンプル旅程を生成します。
- サンプル旅程JSONは `src/data/itineraries/{locale}/{id}.json`（`ja`/`en`）を優先参照し、未存在時のみ従来の `src/data/itineraries/{id}.json` をフォールバックします。

### 3.1b Compose Pipeline (v2, feature-flagged)

Feature flag `NEXT_PUBLIC_ENABLE_COMPOSE_PIPELINE=true` で有効化される新パイプライン。
LLM は「意味」を作り、Google Maps は「現実」を返し、アプリケーションコードが「制約を解く」3層分離。

```
UserInput → [1] Request Normalizer (TS) → NormalizedRequest
         → [2] Semantic Planner (Gemini) → SemanticPlan { candidates[], dayStructure[] }
         → [3] Place Resolver (Places API, flag) → ResolvedPlaceGroup[]
         → [4] Feasibility Scorer (TS) → SelectedStop[]
         → [5] Route Optimizer (greedy + 2-opt) → OptimizedDay[]
         → [6] Timeline Builder (TS) → TimelineDay[]
         → [7] Narrative Renderer (Gemini) → NarrativeDay[]
         → [Adapter] → Itinerary (後方互換)
```

- API: `POST /api/itinerary/compose` (SSE stream)
- 型: `src/types/compose-pipeline.ts`
- 実装: `src/lib/services/compose-pipeline/`
- UI: `ComposeLoadingAnimation` + `ComposeLoadingTips`
- Hook: `useComposeGeneration`
- Places 照合は `ENABLE_COMPOSE_PLACE_RESOLVE` で ON/OFF 制御
- Phase 1 はハバーサイン距離推定 (`distance-estimator.ts`)、Phase 2 で Routes API に差替予定
