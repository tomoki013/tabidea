...
UserInput
  → [POST /api/agent/runs]
  → createRun() → runs
  → [Pass A: normalize] → NormalizedRequest
  → [Pass B: draft_generate] → PlannerDraft
  → [Pass C: draft_format] → DraftPlan
  → [Pass D: rule_score] → EvaluationReport + passGrade
  → [Repair Loop]
      → [Pass E: local_repair] → repaired DraftPlan
      → [Pass D: rule_score]
  → [Pass F: selective_verify] → VerifiedEntity[]
  → [Pass G: timeline_construct] → TimelineState
  → [Pass H: narrative_polish] → NarrativeState
  → [timeline-to-itinerary] → canonical Itinerary
  → persist trip_version / eval_results / run_events
  → [GET /api/agent/runs/:runId/stream] SSE
...
- Deprecated API
  ...
  - `/api/itinerary/plan/*`
  - `/api/plan-generation/session/*`
  - 上記は本体から呼ばれず、deprecated response のみ返す

内部構成:
- 状態機械
  - `created` → `normalized` → `draft_generated` → `draft_formatted` → `draft_scored` → `verification_partial` → `timeline_ready` → `narrative_partial` → `completed`
  - `failed` / `cancelled` は terminal
- パス契約
...
- stream request は terminal まで一気に進めるのではなく、残り budget や recovery 要否に応じて `run.paused` を emit して閉じる
  - フロントは `last-event-id` 付きで `/api/agent/runs/:runId/stream` に再接続し、同じ run を継続する
...
- `run.finished` まで進んだときだけ `trip_version` を保存する
