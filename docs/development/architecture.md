...
2026-04-05 時点の `plan-run` 系 planner は、HTTP route や SSE ではなく `plan_runs` snapshot を唯一の正本とする。`POST /api/v1/plan-runs/[runId]/resume` は 1 slice だけを実行し、lease 取得・`running` 遷移・slice commit・lease 解放は Postgres RPC (`claim_plan_run_execution`, `commit_plan_run_slice`) で原子的に処理する。`GET /api/v1/plan-runs/[runId]/events` は debug/replay 用であり、planner の制御フローには使わない。`plan_run_slices` は append-only の実行履歴として、各 slice の `from_state`, `to_state`, `current_pass_id`, `stop_reason`, `error_code` を保持する。
